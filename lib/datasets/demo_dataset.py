...
class Dataset(pose_dataset.Dataset):
    ...
    def load_render(self):
        ...
        self.K = self.Ks[0]
        if len(self.render_w2c) > len(self.ims) and cfg.perform:
            log(f'will render {len(self.render_w2c)} views on only {len(self.ims)} poses', 'yellow')

    def __getitem__(self, index):
        if cfg.perform:  # ? BUG
            if cfg.test_novel_pose:
                latent_index = cfg.begin_ith_frame + cfg.num_train_frame * cfg.frame_interval + index
            else:
                latent_index = index
        else:
            latent_index = 0
        ...
        # 0->59->0
        ims_index = index % len(self.ims)  # will test some of the novel poses if supported

        img_path = os.path.join(self.data_root, self.ims[ims_index][0])  # loading camera 0
